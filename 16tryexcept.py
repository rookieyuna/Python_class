...
def calc(val):
    ...
    try: #예외발생이 예상되는 지점을 try로 묶는다.
        ...
        if val[0] == 100:
           print(no_var) #선언안된 변수를 출력=>예외발생
        elif val[0]==55:
            result = val[0] / 0 #숫자를 0으로 나눔=>예외발생
            print("결과", result)
    ...
    #예외명을 명시하지 않으면 모든 예외를 처리
    except:
        print('예외가 발생하였습니다')
    #예외가 발생되지 않았을 때 실행되는 블럭
    else:
        print('에러없음^^')
    #try로 진입했을떄 예외발생과 상관없이 무조건 실행
    finally:
        print('sum', sum)
...
try:
    ...
    try: 
        ...
        print("파일 객체를 닫습니다")
        fp.close() 
except IOError: #현재경로에 파일이 없으므로 예외발생
    print('파일에러발생')
...
'''
개발자 정의 예외클래스 생성
    :예외클래스 생성시 Exception 객체를 상속하고
    생성자에서 예외발생 시 출력할 메세지를 정의한다.
'''
print('실행7')
...
def print_gugudan(end_num):
    try:
        #예외를 발생시킬 조건에서 해당 클래스를 raise한다 (throw 역할)
        if end_num > 9:
