...
06String - format() 함수
    ...
    문자열을 표현할 수 있다. {}에 서식을 지정하고 format 함수의
    인자로 값들을 삽입한다.
    형식]
        '{인덱스0}, {인덱스1}'.format(값0, 값1)
        '{x} {y}일'.format(x='오늘은', y=24)
'''
#문자열을 직접 대입
...
#인덱스를 이용해 순서를 바꾸는 것도 가능
str5 = '나이 : {1}, 이름 : {0}'.format('이성계',55)
print(str5)

#하나의 변수를 여러번 사용하는 것도 가능
str6 = '항목1 : {0}, 항목2 : {1}, 항목3 : {0}'.format('서울','부산')
print(str6)

#정수 N자리 표현 : {인덱스:자리수 및 서식문자}
str7 = '정수3자리 : {0:03d}, {1:03d}'.format(12345,12) #=12345, 012
print(str7) #자리의 남는 부분은 0으로 채운다

'''
    파이썬은 문장의 끝에 세미콜론을 사용하지 않으므로 코드를 줄바꿈 할땐
    문장의 끝에 \(역슬러쉬)를 붙여서 연결되는 코드임을 명시해야한다.
'''
#
str8 = '소수점 아래 2자리 : {0:0.2f}, 소수점 아래 5자리 : {1:0.5f}'\
    .format(123.1234567, 3.14) #=123.12, 3.14000
print(str8) #소수의 남는 부분은 0을 채운다

#드문 경우이나 {}자체를 출력해야할때는 중괄호를 겹쳐서 기술한다
str9 = '{{ {0} }}'.format('Python 중괄호 표현')
print("str9=", str9)

#숫자 3자리마다 ,를 삽입하고 싶을 때 쓰는 형식
str10 = 1592000
print(format(str10, ','))
